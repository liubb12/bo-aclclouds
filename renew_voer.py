...
# -*- coding: utf-8 -*-
# ============================================================
# VOER Host 自动续期脚本 (JS 穿透精准击发 + 广告流闭环版)
# ============================================================
...
def get_expire_info(driver) -> str:
  ...
  try:
    ...
    body_text = (
        driver.get_text("body").replace("\u00a0", " ").replace("\u202f", " ")
    )
...
def click_rewarded_watch_ad(driver) -> bool:
  """通过 JS 全面检索并点击模态框内那个绿色 Watch ad 按钮"""
  return driver.execute_script("""
        const allButtons = Array.from(document.querySelectorAll('button'));
        // 查找包含 watch ad 文本的按钮（排除外层 Watch Ads 确认按钮）
        const targetBtn = allButtons.find(b => {
            const txt = (b.innerText || b.textContent || '').trim().toLowerCase();
            return txt === 'watch ad' || (txt.includes('watch') && !txt.includes('ads'));
        });
        if (targetBtn) {
  ...
  while time.time() - start_time < max_wait_sec:
    ...
    try:
      close_btns = driver.find_elements(
          By.XPATH,
          "//*[text()='Close' or translate(text(), 'CLOSE', 'close')='close' or"
          " contains(text(), 'Close') or @id='dismiss-button']",
      )
...
def main():
  ...
  driver = Driver(
      uc=True, headless=False, proxy=uc_proxy, chromium_arg=" ".join(chromium_args)
  )
  ...
  try:
    ...
    modal_opened = False
    watch_ads_xpath = (
        "//button[contains(., 'Watch Ads') or contains(., 'Watch ad')]"
    )
    ...
    if not modal_opened:
      ...
      tg_send(
          "🔴 <b>VOER Host 未能唤起续期弹窗</b>",
          photo_path="failed_extend_modal.png",
      )
      return
    ...
    for current_ad in range(1, 4):
      ...
      clicked = False
      # 最长轮询 35 秒，通过 JS 强行穿透并点击
      for sec in range(35):
        if click_rewarded_watch_ad(driver):
          print(
              f"  🎯 第 {sec + 1} 秒成功通过 JS 穿透点击 [Watch ad] 按钮！",
              flush=True,
          )
          clicked = True
          break
      ...
      if not clicked:
        ...
        print(
            f"  ⚠️ 未能在模态框内等到第 {current_ad} 轮的 [Watch ad] 按钮，已抓取现场截图",
            flush=True,
        )
        break
