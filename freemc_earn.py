...
# -*- coding: utf-8 -*-
# ============================================================
# Freemchosting 自动赚积分脚本 (稳定输入 + 严格过盾版)
# ============================================================
import os
import re
import sys
...
def safe_fill_input(driver, selector_xpath: str, value: str):
    ...
    elem = driver.find_element(By.XPATH, selector_xpath)
    elem.clear()
    time.sleep(0.2)
...
def solve_turnstile_box(driver, max_wait_sec=25) -> bool:
    ...
    while time.time() - start < max_wait_sec:
        ...
        if "成功" in body or "Success" in body:
            print("  ✅ Turnstile 处于已通过状态")
            return True
        try:
            driver.uc_gui_click_captcha()
        except Exception:
            pass
        if recursive_find_and_click(driver, cf_xpaths, current_depth=0, max_depth=3):
            time.sleep(2)
            driver.switch_to.default_content()
            return True
        time.sleep(1.5)
...
def login_freemc(driver):
    ...
    print("🛡️ 处理登录页 Turnstile 并等待通过...", flush=True)
    solve_turnstile_box(driver, max_wait_sec=20)

    for sec in range(20):
        body_text = driver.get_text("body")
        if "成功" in body_text or "Success" in body_text:
            print(f"  ✅ Turnstile 验证通过 (第 {sec + 1} 秒)", flush=True)
            break
        cf_xpaths = ["//input[@type='checkbox']", "//label[contains(@class, 'ctp-checkbox-label')]", "//span[contains(@class, 'mark')]"]
